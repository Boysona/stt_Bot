...
# New: max upload for web interface (MB)
...
# admin_state now stores dicts per admin id (allows pending_broadcast etc)
admin_state = {}
user_transcriptions = {}
...
def admin_menu_callback(call):
    try:
        ...
        if data == "total_users":
            ...
            # Set admin state to awaiting message; the actual sending will be done after admin sends the content
...
@bot.callback_query_handler(func=lambda c: c.data == "admin_confirm_broadcast" and c.from_user.id == ADMIN_ID)
def admin_confirm_broadcast(call):
    try:
        ...
        if not st or "pending_broadcast" not in st:
            ...
            return
        pending = st.pop("pending_broadcast")
...
def prepare_broadcast_message(message):
    try:
        # Save pending message for confirmation
...
# Updated HTML template: renamed to "Media to Text Bot", default language selection respects token-provided 'lang',
# upload hint changed to 300MB and accept includes mp4 and image formats.
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="en">
<head>
                ...
                <form id="transcriptionForm" enctype="multipart/form-data" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <div class="upload-area" id="uploadArea">
                            ...
                            <div class="upload-hint">MP3, WAV, M4A, OGG, WEBM, FLAC, MP4, JPG, PNG, GIF • Max {{ max_mb }}MB</div>
                            <input type="file" id="audioFile" name="file" accept=".mp3,.wav,.m4a,.ogg,.webm,.flac,.mp4,.jpg,.jpeg,.png,.gif" class="d-none" required>
                        </div>
                    </div>
...
def upload_large_file(token):
    ...
    if request.method == 'GET':
        # Pass selected_lang so the page selects the user's bot language by default
