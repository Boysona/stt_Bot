...
admin_state = {}
user_transcriptions = {}
admin_button_state = {"translate": True, "summarize": True, "ads": True}  # Added ads button state
...
def update_group_activity(chat_id: int, chat_title: str = None):
    ...
    groups_collection.update_one(
        {"_id": chat_id_str},
        {"$set": update_data, "$setOnInsert": {"first_seen": now}},
        upsert=True
    )
...
def build_admin_menu():
    ...
    markup.add(
        ...
        InlineKeyboardButton("📊 Total Groups", callback_data="admin_total_groups"),
    )
    markup.add(
    ...
    markup.add(
        InlineKeyboardButton(f"Ads Button: {ads_state}", callback_data="admin_toggle_ads"),
        InlineKeyboardButton("📢 Send Ads", callback_data="admin_send_ads")
    )
    return markup
...
def send_ad_to_users_and_groups(ad_content, content_type):
    """Send ad to all users and groups where bot is admin"""
    try:
        # Send to all users
        ...
        for user in users:
            try:
                if content_type == 'text':
                    bot.send_message(user['_id'], ad_content)
                elif content_type == 'photo':
                    bot.send_photo(user['_id'], ad_content)
                elif content_type == 'video':
                    bot.send_video(user['_id'], ad_content)
                elif content_type == 'audio':
                    bot.send_audio(user['_id'], ad_content)
                elif content_type == 'voice':
                    bot.send_voice(user['_id'], ad_content)
                elif content_type == 'document':
                    bot.send_document(user['_id'], ad_content)
                user_count += 1
                time.sleep(0.1)  # Rate limiting
            except Exception as e:
                logging.error(f"Failed to send ad to user {user['_id']}: {e}")
        ...
        for group in groups:
            try:
                ...
                chat_member = bot.get_chat_member(int(group['_id']), bot.get_me().id)
                if chat_member.status in ['administrator', 'creator']:
                    if content_type == 'text':
                        bot.send_message(group['_id'], ad_content)
                    elif content_type == 'photo':
                        bot.send_photo(group['_id'], ad_content)
                    elif content_type == 'video':
                        bot.send_video(group['_id'], ad_content)
                    elif content_type == 'audio':
                        bot.send_audio(group['_id'], ad_content)
                    elif content_type == 'voice':
                        bot.send_voice(group['_id'], ad_content)
                    elif content_type == 'document':
                        bot.send_document(group['_id'], ad_content)
                    group_count += 1
                    time.sleep(0.1)  # Rate limiting
            except Exception as e:
                logging.error(f"Failed to send ad to group {group['_id']}: {e}")
        
        return user_count, group_count
    except Exception as e:
        logging.error(f"Error in send_ad_to_users_and_groups: {e}")
        return 0, 0
...
def start_handler(message):
    try:
        ...
        if message.chat.type != 'private':
            update_group_activity(message.chat.id, message.chat.title)
...
@bot.callback_query_handler(func=lambda c: c.data and c.data in ("admin_total_users", "admin_total_groups", "admin_toggle_translate", "admin_toggle_summarize", "admin_toggle_ads", "admin_send_ads") and c.from_user.id == ADMIN_ID)
def admin_menu_callback(call):
    ...
    try:
        ...
        if data == "total_users":
            ...
            bot.send_message(chat_id, f"👥 Total groups: {total_groups}")
            ...
            bot.edit_message_text(f"⚙️ Admin Panel\n\nAds button is now: {status}",
                                  chat_id=chat_id, message_id=call.message.message_id,
                                  reply_markup=build_admin_menu())
            bot.answer_callback_query(call.id, f"Ads button is now {status}", show_alert=True)
        elif data == "send_ads":
            if not admin_button_state["ads"]:
                bot.answer_callback_query(call.id, "❌ Ads feature is currently disabled", show_alert=True)
                return
                
            admin_state[ADMIN_ID] = "waiting_for_ad_content"
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton("❌ Cancel", callback_data="admin_cancel_ad"))
            bot.send_message(chat_id, "📢 Please send the ad content (text, photo, video, audio, voice, or document):", reply_markup=markup)
...
@bot.callback_query_handler(func=lambda c: c.data == "admin_cancel_ad" and c.from_user.id == ADMIN_ID)
def cancel_ad_callback(call):
    ...
    try:
        ...
        bot.delete_message(call.message.chat.id, call.message.message_id)
        bot.answer_callback_query(call.id, "❌ Ad sending cancelled")
    except Exception:
        logging.exception("Error in cancel_ad_callback")
        try:
            bot.answer_callback_query(call.id, "❌ Error", show_alert=True)
        except Exception:
            pass

@bot.message_handler(content_types=['text', 'photo', 'video', 'audio', 'voice', 'document'], func=lambda message: message.from_user.id == ADMIN_ID and ADMIN_ID in admin_state and admin_state[ADMIN_ID] == "waiting_for_ad_content")
def handle_ad_content(message):
    try:
        ...
        if message.text:
            ...
            content_type = 'photo'
            ad_content = message.photo[-1].file_id
        elif message.video:
            content_type = 'video'
            ad_content = message.video.file_id
        elif message.audio:
            content_type = 'audio'
            ad_content = message.audio.file_id
        elif message.voice:
            content_type = 'voice'
            ad_content = message.voice.file_id
        elif message.document:
            content_type = 'document'
            ad_content = message.document.file_id
        
        if not content_type or not ad_content:
            bot.send_message(message.chat.id, "❌ Unsupported content type. Please send text, photo, video, audio, voice, or document.")
            return
        
        # Ask for confirmation
        markup = InlineKeyboardMarkup()
        markup.add(
            InlineKeyboardButton("✅ Confirm", callback_data=f"admin_confirm_ad|{content_type}|{ad_content}"),
            InlineKeyboardButton("❌ Cancel", callback_data="admin_cancel_ad")
        )
        
        if content_type == 'text':
            preview_msg = f"📢 Ad Preview:\n\n{ad_content[:200]}{'...' if len(ad_content) > 200 else ''}"
        else:
            preview_msg = f"📢 Ad Preview: {content_type.upper()} content"
            
        bot.send_message(message.chat.id, f"{preview_msg}\n\nSend this ad to all users and groups?", reply_markup=markup)
        del admin_state[ADMIN_ID]
        
    except Exception:
        logging.exception("Error in handle_ad_content")
        bot.send_message(message.chat.id, "❌ Error processing ad content")

@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("admin_confirm_ad|") and c.from_user.id == ADMIN_ID)
def confirm_ad_callback(call):
    log_callback(call)
    try:
        _, content_type, ad_content = call.data.split("|", 2)
        
        # Send processing message
        processing_msg = bot.send_message(call.message.chat.id, "🔄 Sending ad to all users and groups...")
        
        # Send ad in background
        def send_ad_background():
            user_count, group_count = send_ad_to_users_and_groups(ad_content, content_type)
        ...
        threading.Thread(target=send_ad_background).start()
        bot.answer_callback_query(call.id)
        
    except Exception:
        logging.exception("Error in confirm_ad_callback")
        try:
            bot.answer_callback_query(call.id, "❌ Error sending ad", show_alert=True)
        except Exception:
            pass
