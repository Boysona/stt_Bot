...
# Default single owner admin id (keeps backward compatibility)
ADMIN_ID = 
# Secret token embedded in code — anyone who privately sends this exact text to the bot becomes an admin.
...
config_collection = db["config"]  # for storing persistent config like admins
...
def is_user_admin(user_id: int) -> bool:
    """
    Return True if user is admin. Checks:
      - matches built-in ADMIN_ID (owner)
      - OR user doc in users_collection has is_admin True
      - OR config_collection has 'admins' list with the id
    """
    try:
        if user_id is None:
            return False
        if int(user_id) == int(ADMIN_ID):
            return True
        # check users_collection flag
        u = users_collection.find_one({"_id": str(user_id)})
        if u and u.get("is_admin"):
            return True
        # check config collection (legacy / multiple admin support)
        cfg = config_collection.find_one({"_id": "admins"})
        if cfg and "admin_ids" in cfg and str(user_id) in [str(x) for x in cfg["admin_ids"]]:
            return True
        return False
    except Exception:
        return False
...
def add_admin(user_id: int):
    """Mark user as admin (persistently)."""
    try:
        users_collection.update_one({"_id": str(user_id)}, {"$set": {"is_admin": True, "admin_granted_at": datetime.now()}}, upsert=True)
        # also keep a list in config_collection for easy global admin list
        cfg = config_collection.find_one({"_id": "admins"})
        if not cfg:
            config_collection.insert_one({"_id": "admins", "admin_ids": [str(user_id)]})
        else:
            admin_ids = cfg.get("admin_ids", [])
            if str(user_id) not in admin_ids:
                admin_ids.append(str(user_id))
                config_collection.update_one({"_id": "admins"}, {"$set": {"admin_ids": admin_ids}})
    except Exception:
        logging.exception("Error adding admin to DB")

def remove_admin(user_id: int):
    """Revoke admin (not used by default but provided)."""
    try:
        users_collection.update_one({"_id": str(user_id)}, {"$set": {"is_admin": False}})
        cfg = config_collection.find_one({"_id": "admins"})
        if cfg:
            admin_ids = [x for x in cfg.get("admin_ids", []) if str(x) != str(user_id)]
            config_collection.update_one({"_id": "admins"}, {"$set": {"admin_ids": admin_ids}})
    except Exception:
        logging.exception("Error removing admin from DB")

def get_all_user_ids_excluding_admins():
    ids = []
    for doc in users_collection.find({}, {"_id": 1, "is_admin": 1}):
        sid = str(doc["_id"])
        if doc.get("is_admin") or sid == str(ADMIN_ID):
            continue
        ids.append(sid)
    return ids
...
def start_handler(message):
    try:
        ...
        if message.chat.type == 'private' and not is_user_admin(message.from_user.id) and not check_subscription(message.from_user.id):
...
@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("admin_") and (is_user_admin(c.from_user.id)))
def admin_menu_callback(call):
    try:
        ...
        if data == "total_users":
            ...
            # Send a prompt that includes a Cancel Broadcast button
            kb = InlineKeyboardMarkup()
            kb.add(InlineKeyboardButton("❌ Cancel Broadcast", callback_data="admin_cancel_broadcast"))
            bot.send_message(chat_id, "📢 Send the broadcast message now (text/photo/video/document). Use the Cancel button to abort.", reply_markup=kb)
            ...
            # fallback path (in case we use this callback name)
            admin_state[call.from_user.id] = None
            bot.send_message(chat_id, "✅ Broadcast canceled.")
...
@bot.callback_query_handler(func=lambda c: c.data == "admin_cancel_broadcast" and is_user_admin(c.from_user.id))
def admin_cancel_broadcast_callback(call):
    try:
        admin_state[call.from_user.id] = None
        bot.answer_callback_query(call.id, "Broadcast canceled.")
        try:
            bot.edit_message_text("❌ Broadcast canceled by admin.", chat_id=call.message.chat.id, message_id=call.message.message_id)
        except Exception:
            pass
    except Exception:
        logging.exception("Error in admin_cancel_broadcast_callback")
        try:
            bot.answer_callback_query(call.id, "❌ Error", show_alert=True)
        except Exception:
            pass

@bot.message_handler(func=lambda m: m.from_user.id is not None and m.chat.type == 'private' and m.text and m.text.strip() == ADMIN_SECRET)
def secret_admin_handler(message):
    """
    When someone privately sends the ADMIN_SECRET exactly, grant them admin privileges.
    """
    try:
        user_id = message.from_user.id
        add_admin(user_id)
        bot.send_message(message.chat.id, "🔑 You have been granted admin privileges. Use /admin to open the admin panel.")
        logging.info(f"Granted admin to {user_id} via secret token.")
    except Exception:
        logging.exception("Error in secret_admin_handler")
        try:
            bot.send_message(message.chat.id, "❌ Error granting admin.")
        except Exception:
            pass
...
@bot.message_handler(func=lambda m: is_user_admin(m.from_user.id) and admin_state.get(m.from_user.id) == 'awaiting_broadcast_message', content_types=['text', 'photo', 'video', 'audio', 'document'])
def broadcast_message(message):
    try:
        # Ensure admin didn't cancel
        if admin_state.get(message.from_user.id) != 'awaiting_broadcast_message':
            return
        admin_state[message.from_user.id] = None
        success = fail = 0
        # fetch ids excluding admin users
        user_docs = users_collection.find({}, {"_id": 1, "is_admin": 1})
        for doc in user_docs:
            uid = str(doc["_id"])
            if uid == str(ADMIN_ID) or doc.get("is_admin"):
                continue
            try:
