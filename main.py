...
admin_sessions = {} # Simple in-memory store for admin sessions
...
def make_lang_keyboard():
    ...
    for label, code in LANG_OPTIONS:
        buttons.append(InlineKeyboardButton(text=label, callback_data=f"lang:{code}"))
...
def get_user_lang(user_id):
    """Get user's language from DB or in-memory cache, defaulting to 'en'."""
    user_data = users_collection.find_one({"user_id": user_id})
...
def get_admin_keyboard():
    """Returns the admin keyboard."""
    kb = ReplyKeyboardMarkup(resize_keyboard=True)
    kb.row(KeyboardButton("Send Broadcast"))
    kb.row(KeyboardButton("Total Users"))
    return kb
...
def handle_admin_start(message):
    chat_id = message.chat.id
    msg = bot.send_message(chat_id, "Please enter the admin password:")
...
def process_admin_password(message):
    ...
    if password == ADMIN_PASSWORD:
        ...
        bot.send_message(chat_id, "✅ Login successful. Welcome, Admin!", reply_markup=get_admin_keyboard())
    else:
        bot.send_message(chat_id, "❌ Incorrect password. Access denied.")

@bot.message_handler(func=lambda message: message.text in ["Send Broadcast", "Total Users"])
def handle_admin_commands(message):
    chat_id = message.chat.id
    if admin_sessions.get(chat_id):
        if message.text == "Send Broadcast":
            msg = bot.send_message(chat_id, "Please send the message you want to broadcast to all users:")
            bot.register_next_step_handler(msg, process_broadcast_message)
        elif message.text == "Total Users":
            total_users = users_collection.count_documents({})
            bot.send_message(chat_id, f"📊 Total number of users: {total_users}")
    else:
        bot.send_message(chat_id, "❌ You are not authorized to use this command. Please use /admin to log in.")

def process_broadcast_message(message):
    chat_id = message.chat.id
    broadcast_text = message.text
    if not broadcast_text:
        bot.send_message(chat_id, "❌ Broadcast message cannot be empty. Please try again.")
        return
    ...
    bot.send_message(chat_id, "🚀 Starting broadcast...")
    for user in users:
        try:
            bot.send_message(user["user_id"], broadcast_text)
            sent_count += 1
        except telebot.apihelper.ApiTelegramException as e:
            # Handle users who have blocked the bot
            print(f"Failed to send message to user {user['user_id']}: {e}")
            failed_count += 1
        except Exception as e:
            print(f"An unexpected error occurred for user {user['user_id']}: {e}")
            failed_count += 1
