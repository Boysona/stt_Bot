...
MAX_WEB_UPLOAD_MB = 300

client = MongoClient(MONGO_URI)
db = client[DB_NAME]
users_collection = db["users"]
tokens_collection = db["tokens"]
groups_collection = db["groups"]  # stores group/supergroup ids where bot is admin
...
admin_state = {}
user_transcriptions = {}
admin_button_state = {"translate": True, "summarize": True, "ads": True}  # added 'ads' toggle
...
def build_admin_menu():
    ...
    ads_state = "✅ Open" if admin_button_state["ads"] else "🚫 Closed"
    markup.add(
    ...
    markup.add(
        InlineKeyboardButton(f"📣 Send Ads: {ads_state}", callback_data="admin_send_ads"),
        InlineKeyboardButton("🔒 Ads Control", callback_data="admin_toggle_ads")
    )
    return markup
...
@bot.callback_query_handler(func=lambda c: c.data and c.data in ("admin_total_users", "admin_toggle_translate", "admin_toggle_summarize", "admin_send_ads", "admin_toggle_ads") and c.from_user.id == ADMIN_ID)
def admin_menu_callback(call):
    ...
    try:
        ...
        if data == "total_users":
            ...
            # Check central toggle
            if not admin_button_state.get("ads", True):
                bot.answer_callback_query(call.id, "❌ Send Ads is currently disabled (Ads Control).", show_alert=True)
                return
            # Show ad type options
            markup = InlineKeyboardMarkup(row_width=2)
            markup.add(
                InlineKeyboardButton("Text", callback_data="admin_select_adstype|text"),
                InlineKeyboardButton("Document", callback_data="admin_select_adstype|document")
            )
            markup.add(
                InlineKeyboardButton("Audio", callback_data="admin_select_adstype|audio"),
                InlineKeyboardButton("Voice", callback_data="admin_select_adstype|voice")
            )
            markup.add(
                InlineKeyboardButton("Video", callback_data="admin_select_adstype|video"),
                InlineKeyboardButton("Cancel", callback_data="admin_cancel_ads")
            )
            bot.send_message(chat_id, "📣 Choose ad content type to send to all users & groups:", reply_markup=markup)
            bot.answer_callback_query(call.id)
        elif data == "toggle_ads":
            admin_button_state["ads"] = not admin_button_state["ads"]
            status = "Open" if admin_button_state["ads"] else "Closed"
            bot.edit_message_text(f"⚙️ Admin Panel\n\nSend Ads is now: {status}",
                                  chat_id=chat_id, message_id=call.message.message_id,
                                  reply_markup=build_admin_menu())
            bot.answer_callback_query(call.id, f"Send Ads is now {status}", show_alert=True)
        bot.answer_callback_query(call.id)
    except Exception:
...
# Handler for selecting ad type (admin)
@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("admin_select_adstype|") and c.from_user.id == ADMIN_ID)
def admin_select_adstype(call):
    log_callback(call)
    try:
        ...
        if len(parts) < 2:
            ...
            return
        ad_type = parts[1]
        admin_state[str(call.from_user.id)] = {"mode": "send_ads_waiting", "type": ad_type}
        # Prompt admin to send the content (text or media)
        if ad_type == "text":
            bot.send_message(call.message.chat.id, "✍️ Send the text message you want to broadcast to ALL users and groups. When ready, send it here.")
        else:
            bot.send_message(call.message.chat.id, f"📎 Send the {ad_type} (attach file or voice) now. You may include a caption if you want.")
        bot.answer_callback_query(call.id)
    except Exception:
        logging.exception("Error in admin_select_adstype")
        try:
            bot.answer_callback_query(call.id, "❌ Error", show_alert=True)
        except Exception:
            pass
...
# Cancel ad flow
@bot.callback_query_handler(func=lambda c: c.data == "admin_cancel_ads" and c.from_user.id == ADMIN_ID)
def admin_cancel_ads(call):
    try:
        admin_state.pop(str(call.from_user.id), None)
        bot.answer_callback_query(call.id, "Ad send cancelled.", show_alert=True)
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except Exception:
            pass
    except Exception:
        logging.exception("Error cancelling ad")

# When admin sends the content (text or media) while in send_ads_waiting mode
@bot.message_handler(func=lambda m: m.from_user and m.from_user.id == ADMIN_ID, content_types=['text','document','audio','voice','video','photo'])
def admin_receive_ads_content(message):
    try:
        uid = str(message.from_user.id)
        state = admin_state.get(uid)
        if not state or state.get("mode") != "send_ads_waiting":
            # Not in ad-sending flow; ignore here (other handlers)
            return
        ad_type = state.get("type")
        # extract content depending on type
        ad_payload = {"type": ad_type, "file_id": None, "caption": None, "text": None}
        if ad_type == "text":
            if not message.text:
                bot.send_message(message.chat.id, "Please send text content for the ad.")
                return
            ad_payload["text"] = message.text
        elif ad_type == "document":
            if not message.document:
                bot.send_message(message.chat.id, "Please attach a document file.")
                return
            ad_payload["file_id"] = message.document.file_id
            ad_payload["caption"] = message.caption or ""
        elif ad_type == "audio":
            if not message.audio:
                bot.send_message(message.chat.id, "Please attach an audio file.")
                return
            ad_payload["file_id"] = message.audio.file_id
            ad_payload["caption"] = message.caption or ""
        elif ad_type == "voice":
            if not message.voice:
                bot.send_message(message.chat.id, "Please send a voice message.")
                return
            ad_payload["file_id"] = message.voice.file_id
            ad_payload["caption"] = message.caption or ""
        elif ad_type == "video":
            if message.video:
                ad_payload["file_id"] = message.video.file_id
                ad_payload["caption"] = message.caption or ""
            elif message.photo:
                # allow photo to be used as a video placeholder? better insist on video
                bot.send_message(message.chat.id, "Please send a video file, not a photo.")
                return
            else:
                bot.send_message(message.chat.id, "Please attach a video file.")
                return
        else:
            bot.send_message(message.chat.id, "Unknown ad type. Cancel and try again.")
            admin_state.pop(uid, None)
            return

        # Save payload into admin_state and ask for confirmation
        admin_state[uid]["payload"] = ad_payload

        preview_text = "Preview:\n"
        if ad_type == "text":
            preview_text += ad_payload["text"][:1500] + ("\n\n(Truncated preview)" if len(ad_payload["text"]) > 1500 else "")
        else:
            preview_text += f"Type: {ad_type}\nCaption: {ad_payload.get('caption','')}"

        markup = InlineKeyboardMarkup(row_width=2)
        markup.add(
            InlineKeyboardButton("✅ Confirm & Send", callback_data="admin_confirm_ads"),
            InlineKeyboardButton("❌ Cancel", callback_data="admin_cancel_ads")
        )
        bot.send_message(message.chat.id, preview_text, reply_markup=markup)
    except Exception:
        logging.exception("Error receiving ad content")
        try:
            bot.send_message(message.chat.id, "An error occurred while processing your ad content.")
        except Exception:
            pass

# Confirm and start broadcasting
@bot.callback_query_handler(func=lambda c: c.data == "admin_confirm_ads" and c.from_user.id == ADMIN_ID)
def admin_confirm_and_send(call):
    log_callback(call)
    try:
        uid = str(call.from_user.id)
        state = admin_state.get(uid)
        if not state or "payload" not in state:
            bot.answer_callback_query(call.id, "No ad content found. Please start again.", show_alert=True)
            return
        payload = state["payload"]
        ad_type = payload["type"]
        bot.answer_callback_query(call.id, "Starting broadcast... This may take some time.", show_alert=True)
        # send a starting notification message
        try:
            bot.edit_message_text("📣 Broadcasting ads... please wait while messages are sent.", chat_id=call.message.chat.id, message_id=call.message.message_id)
        except Exception:
            try:
                bot.send_message(call.message.chat.id, "📣 Broadcasting ads... please wait while messages are sent.")
            except Exception:
                pass

        # Build target lists robustly (skip invalid/non-int ids)
        user_cursor = users_collection.find({}, {"_id": 1})
        group_cursor = groups_collection.find({}, {"_id": 1})

        user_ids = []
        for u in user_cursor:
            uid_val = u.get("_id")
            try:
                # _id may be a numeric string like "12345" or an int-like value; try to convert
                user_ids.append(int(uid_val))
            except Exception:
                logging.warning(f"Skipping user id that cannot be converted to int: {_repr := uid_val!r}")
                continue
        ...
        for g in group_cursor:
            gid_val = g.get("_id")
            try:
                group_ids.append(int(gid_val))
            except Exception:
                logging.warning(f"Skipping group id that cannot be converted to int: {_repr := gid_val!r}")
                continue

        # If no targets, notify admin and exit
        total_targets = len(user_ids) + len(group_ids)
        if total_targets == 0:
            bot.send_message(call.message.chat.id, "No targets found to broadcast to (users/groups list is empty).")
            admin_state.pop(uid, None)
            return
        ...
        # Helper to send to one chat
        def send_to_chat(chat_id):
            nonlocal success_count, fail_count, fail_examples
            try:
                if ad_type == "text":
                    bot.send_message(chat_id, payload["text"])
                elif ad_type == "document":
                    bot.send_document(chat_id, payload["file_id"], caption=payload.get("caption") or "")
                elif ad_type == "audio":
                    bot.send_audio(chat_id, payload["file_id"], caption=payload.get("caption") or "")
                elif ad_type == "voice":
                    bot.send_voice(chat_id, payload["file_id"], caption=payload.get("caption") or "")
                elif ad_type == "video":
                    bot.send_video(chat_id, payload["file_id"], caption=payload.get("caption") or "")
                else:
                    bot.send_message(chat_id, "Unsupported ad type.")
                    with counters_lock:
                        fail_count += 1
                    return
                with counters_lock:
                ...
                logging.exception("Failed sending ad to %s: %s", chat_id, e)
                with counters_lock:
                    fail_count += 1
                    if len(fail_examples) < 5:
                        # include a short exception snippet
                        fail_examples.append(f"{chat_id}: {str(e)[:200]}")
            # light throttle to avoid hitting Telegram limits
            time.sleep(0.08)

        # Start broadcast to users
        for uid_target in user_ids:
            try:
                send_to_chat(uid_target)
            except Exception:
                logging.exception("Error while sending to user_id %s", uid_target)
                # don't abort the whole broadcast; continue

        # Then broadcast to groups
        for gid in group_ids:
            try:
                send_to_chat(gid)
            except Exception:
                logging.exception("Error while sending to group_id %s", gid)
                # continue on failure

        # Report to admin
        summary = (
            f"📣 Broadcast complete.\n\n"
            f"Total targets attempted: {total_targets}\n"
        ...
        # send a helpful message back to the admin (but avoid leaking big tracebacks)
        try:
            bot.send_message(call.message.chat.id, f"❌ An error occurred while broadcasting ads. See logs. Error: {str(exc)[:400]}")
        except Exception:
            pass
...
def handle_media_common(message, target_bot: telebot.TeleBot):
    uid = str(message.from_user.id)
    update_user_activity(message.from_user.id)
    # track group chats where bot is admin
    try:
        if message.chat and message.chat.type != 'private':
            try:
                # check bot's membership in this chat
                ...
                if bot_member and bot_member.status in ['administrator', 'creator']:
                    ...
                    groups_collection.update_one({"_id": str(message.chat.id)}, {"$set": {"last_seen": datetime.now(), "type": message.chat.type}}, upsert=True)
            except Exception:
                # ignore failures to check membership
                pass

    except Exception:
        logging.exception("Error tracking groups")
...
# Track groups where bot is admin by observing messages in non-private chats.
@bot.message_handler(func=lambda m: m.chat is not None and m.chat.type != 'private', content_types=['text','voice','audio','video','document','photo'])
def track_groups_on_message(m):
    try:
        # if this is a group/supergroup, try to check if bot is admin
        chat_id = m.chat.id
        try:
            bot_member = bot.get_chat_member(chat_id, bot.get_me().id)
            if bot_member and bot_member.status in ['administrator', 'creator']:
                groups_collection.update_one({"_id": str(chat_id)}, {"$set": {"last_seen": datetime.now(), "title": m.chat.title or "", "type": m.chat.type}}, upsert=True)
        except Exception:
            # ignore membership check failure
            pass
        # also record the user who sent the message to users collection (so we grow the list)
        if m.from_user and m.from_user.id:
            update_user_activity(m.from_user.id)
    except Exception:
        logging.exception("Error tracking group or user activity")
    ...
    <script>
        class TranscriptionApp {
            ...
            async handleSubmit(e) {
                e.preventDefault();
                const formEl = document.getElementById('transcriptionForm');
                    ...
                    if (response.ok) {
                        statusContainer.innerHTML = `
                            <div class="close-notice">
                                ...
                                File uploaded successfully! Your transcription will be ready shortly. You can close this tab now.
                            </div>
                        `;
                    } else {
    ...
    def bytes_gen(b):
        ...
        while True:
            ...
            if not chunk:
                break
            yield chunk
    def process_uploaded_file(chat_id_inner, lang_inner, b, upload_msg_id_inner):
        try:
            ...
            final_msg = bot.send_message(chat_id_inner, "Your transcription is ready.", reply_to_message_id=upload_msg_id_inner)
            ...
            if admin_button_state["translate"] or admin_button_state["summarize"]:
                ...
                markup = None
            if len(text) > 4000:
                user_transcriptions.setdefault(str(chat_id_inner), {})[final_msg_id] = text
                threading.Thread(target=delete_transcription_later, args=(str(chat_id_inner), final_msg_id), daemon=True).start()
                fobj = io.BytesIO(text.encode("utf-8"))
                fobj.name = "transcription.txt"
                bot.send_document(chat_id_inner, fobj, caption="Your transcription is ready.", reply_markup=markup, reply_to_message_id=final_msg_id)
            else:
                user_transcriptions.setdefault(str(chat_id_inner), {})[final_msg_id] = text
                threading.Thread(target=delete_transcription_later, args=(str(chat_id_inner), final_msg_id), daemon=True).start()
                bot.send_message(chat_id_inner, text or "No transcription text was returned.", reply_markup=markup, reply_to_message_id=final_msg_id)
            increment_processing_count(str(chat_id_inner), "stt")
        except Exception:
            logging.exception("Error transcribing uploaded file")
            try:
                bot.send_message(chat_id_inner, "Error occurred while transcribing the uploaded file.", reply_to_message_id=upload_msg_id_inner)
            except Exception:
                pass
        finally:
            try:
                bot.delete_message(chat_id_inner, upload_msg_id_inner)
            except Exception:
                pass
    threading.Thread(target=process_uploaded_file, args=(chat_id, lang, file_bytes, upload_msg_id), daemon=True).start()
    return "<h3>Upload accepted. Your transcription will be sent to your Telegram chat shortly.</h3>"
