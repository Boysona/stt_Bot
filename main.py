...
# Increase default request timeouts (seconds)
DEFAULT_REQUEST_TIMEOUT = 300  # 5 minutes for long requests (Gemini/AssemblyAI polling etc.)
ASSEMBLYAI_UPLOAD_TIMEOUT = 3600  # keep long for big uploads
...
def ask_gemini(text: str, instruction: str, timeout=DEFAULT_REQUEST_TIMEOUT) -> str:
    url = (
...
def assemblyai_upload_from_stream(stream_iterable):
    ...
    resp = requests.post(upload_url, headers=headers, data=stream_iterable, timeout=ASSEMBLYAI_UPLOAD_TIMEOUT)
    resp.raise_for_status()
    return resp.json().get("upload_url")

def assemblyai_upload_file_path(file_path, chunk_size=256*1024):
    """
    Upload a local file to AssemblyAI via streaming (without loading whole file into memory).
    Returns the upload_url provided by AssemblyAI.
    """
    upload_url = "https://api.assemblyai.com/v2/upload"
    headers = {"authorization": ASSEMBLYAI_API_KEY}
    def file_iterator():
        with open(file_path, 'rb') as fh:
            while True:
                ...
                if not chunk:
                    break
                yield chunk
    resp = requests.post(upload_url, headers=headers, data=file_iterator(), timeout=ASSEMBLYAI_UPLOAD_TIMEOUT)
    resp.raise_for_status()
    return resp.json().get("upload_url")
...
def create_transcript_and_wait(audio_url: str, language_code: str = None, speech_model: str = None, poll_interval=2):
    ...
    resp = requests.post(create_url, headers=headers, json=data, timeout=DEFAULT_REQUEST_TIMEOUT)
    resp.raise_for_status()
    job = resp.json()
    job_id = job.get("id")
    get_url = f"{create_url}/{job_id}"
    while True:
        r = requests.get(get_url, headers={"authorization": ASSEMBLYAI_API_KEY}, timeout=DEFAULT_REQUEST_TIMEOUT)
        r.raise_for_status()
        status = r.json()
...
def telegram_file_stream(file_url, chunk_size=256*1024):
    with requests.get(file_url, stream=True, timeout=DEFAULT_REQUEST_TIMEOUT) as r:
        r.raise_for_status()
...
@bot.callback_query_handler(func=lambda c: c.data and c.data in ("admin_total_users", "admin_toggle_translate", "admin_toggle_summarize", "admin_broadcast") and c.from_user.id == ADMIN_ID)
def admin_menu_callback(call):
    ...
    try:
        ...
        if data == "total_users":
            ...
            bot.send_message(chat_id, "Send the message you want to broadcast (text, video, audio, voice, document).")
...
def process_broadcast_message(message):
    try:
        if message.content_type not in ['text', 'video', 'audio', 'voice', 'document']:
            bot.send_message(message.chat.id, "Unsupported message type. Please send text, video, audio, voice, or document.")
            return
        ...
        confirm_text = "Confirm broadcast to all users and groups?"
        markup = InlineKeyboardMarkup()
        markup.row(
            InlineKeyboardButton("Yes", callback_data=f"broadcast_confirm|{message.message_id}"),
            InlineKeyboardButton("Cancel", callback_data="broadcast_cancel")
        )
        bot.send_message(message.chat.id, confirm_text, reply_markup=markup)

        with memory_lock:
            in_memory_data["broadcast_data"] = broadcast_data

    except Exception:
        logging.exception("Error in process_broadcast_message")
        bot.send_message(message.chat.id, "Error processing broadcast message.")

@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith(("broadcast_confirm|", "broadcast_cancel")))
def handle_broadcast_confirmation(call):
    try:
        ...
        with memory_lock:
            broadcast_data = in_memory_data.get("broadcast_data")

        if not broadcast_data:
            bot.edit_message_text("Broadcast data expired. Please start over.", call.message.chat.id, call.message.message_id)
            return
        ...
        def send_broadcast():
            try:
                ...
                for user in users:
                    try:
                        send_message_to_chat(user['_id'], broadcast_data)
                        total_sent += 1
                    except Exception:
                        total_failed += 1
                    time.sleep(0.1)
                ...
                for group in groups:
                    try:
                        send_message_to_chat(group['_id'], broadcast_data)
                        total_sent += 1
                    except Exception:
                        total_failed += 1
                    time.sleep(0.1)

                bot.send_message(call.message.chat.id, f"Broadcast completed. Sent: {total_sent}, Failed: {total_failed}")
            except Exception:
                logging.exception("Error in broadcast thread")
                bot.send_message(call.message.chat.id, "Error during broadcast.")

        threading.Thread(target=send_broadcast).start()

    except Exception:
        logging.exception("Error in handle_broadcast_confirmation")
        bot.send_message(call.message.chat.id, "Error confirming broadcast.")

def send_message_to_chat(chat_id, broadcast_data):
    try:
        if broadcast_data['content_type'] == 'text':
            bot.send_message(chat_id, broadcast_data['text'])
        else:
            file_id = broadcast_data['file_id']
            caption = broadcast_data.get('caption', '')

            if broadcast_data['content_type'] == 'video':
...
def handle_new_chat_members(message):
    try:
        if message.new_chat_members[0].id == bot.get_me().id:
            ...
            bot.send_message(message.chat.id, "Thanks for adding me! I'm ready to transcribe your media files.")
    except Exception:
...
<head>
    ...
    <style>
        /* CSS omitted for brevity in code block — same as previous template */
    </style>
</head>
<body>
    <!-- HTML omitted for brevity — same as previous template -->
</body>
</html>
"""
...
def upload_large_file(token):
    """
    Web upload endpoint: Save uploaded file to disk immediately (fast disk copy),
    return accepted response to client, and process the file in a background thread
    that streams the file to AssemblyAI. This avoids loading the file into memory.
    """
    try:
    ...
    # Save to temp file asap (fast disk copy) to avoid reading whole into memory
    try:
        # Build a safe temp filename with original extension if available
        original_filename = getattr(file, 'filename', '') or ''
        _, ext = os.path.splitext(original_filename)
        fd, tmp_path = tempfile.mkstemp(suffix=ext or '')
        os.close(fd)  # we'll use file.save which writes to the path
        file.save(tmp_path)
    except Exception:
        logging.exception("Failed to save uploaded file to temp disk")
        # Ensure cleanup if something partially wrote
        try:
    ...
    # File size check (after saving)
    try:
        size = os.path.getsize(tmp_path)
        if size > MAX_WEB_UPLOAD_MB * 1024 * 1024:
            try:
                os.remove(tmp_path)
            except Exception:
                pass
        ...
        logging.exception("Failed to stat temp file")
        try:
            os.remove(tmp_path)
        except Exception:
            pass
    ...
    def process_uploaded_file_path(chat_id_inner, lang_inner, path):
        try:
            logging.info(f"Background processing started for {path} (chat {chat_id_inner})")
            # Upload file to AssemblyAI by streaming from disk
            upload_url = assemblyai_upload_file_path(path)
            logging.info(f"Uploaded to AssemblyAI: {upload_url}")
            ...
            try:
                if len(text) > 4000:
                    ...
                    sent_msg = bot.send_document(chat_id_inner, fobj, caption="Transcription too long. Here's the complete text in a file.")
                else:
                    sent_msg = bot.send_message(chat_id_inner, text or "No transcription text was returned.")
            except Exception:
                logging.exception("Failed to send transcription message to user")
                try:
                    bot.send_message(chat_id_inner, "Error sending transcription message. The transcription completed but could not be delivered as a message.")
                except Exception:
                    pass
                return

            # Attach translate/summarize buttons using actual message id
            try:
                if admin_button_state["translate"] or admin_button_state["summarize"]:
                    markup = InlineKeyboardMarkup()
                    if admin_button_state["translate"]:
                        markup.add(InlineKeyboardButton("Translate", callback_data=f"btn_translate|{sent_msg.message_id}"))
                    if admin_button_state["summarize"]:
                        markup.add(InlineKeyboardButton("Summarize", callback_data=f"btn_summarize|{sent_msg.message_id}"))
                    try:
                        bot.edit_message_reply_markup(chat_id=chat_id_inner, message_id=sent_msg.message_id, reply_markup=markup)
                    except Exception:
                        logging.info("Could not attach inline buttons to the sent message.")
                else:
                    markup = None
            except Exception:
                logging.exception("Error building or attaching markup")

            # store transcription for later translate/summarize actions
            try:
                ...
                logging.exception("Failed to store transcription in memory")
            ...
            try:
                if os.path.exists(path):
                    os.remove(path)
                    logging.info(f"Temp file removed: {path}")
            except Exception:
                logging.exception("Failed to remove temp file")

    # Start background thread to process the temp file and immediately respond to user
    threading.Thread(target=process_uploaded_file_path, args=(chat_id, lang, tmp_path), daemon=True).start()
