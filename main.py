...
for display, code in LANG_OPTIONS:
    ...
    native = parts[1] if len(parts) > 1 else display
    STT_LANGUAGES[native] = {"code": code, "emoji": emoji, "native": native}
...
def send_subscription_message(chat_id: int):
    ...
    if chat and chat.type == 'private':
        ...
        markup.add(
            InlineKeyboardButton("🔓 Join the group to unlock", url=f"https://t.me/{REQUIRED_CHANNEL.lstrip('@')}")
        )
        bot.send_message(chat_id, "🔒 Access Locked. To use this Bot, please join our group first. Tap the button below to join and then send /start.", reply_markup=markup)
...
def start_handler(message):
    try:
        ...
        if message.chat.type == 'private' and str(message.from_user.id) != str(ADMIN_ID) and not check_subscription(message.from_user.id):
            ...
            return
        bot.send_message(message.chat.id, "Set transcription language using the below buttons:", reply_markup=build_start_language_keyboard())
    except Exception:
...
def gemini_generate(prompt, max_output_tokens=800, temperature=0.2):
    url = f"https://generativelanguage.googleapis.com/v1beta2/models/text-bison-001:generateText?key={GEMINI_API_KEY}"
    payload = {"prompt": {"text": prompt}, "maxOutputTokens": max_output_tokens, "temperature": temperature}
    r = requests.post(url, json=payload, timeout=120)
    r.raise_for_status()
    j = r.json()
    out = ""
    try:
        out = j.get("candidates", [{}])[0].get("output", "")
    except Exception:
        out = j.get("output", "")
    return out

def gemini_translate(text, target_lang_name, target_code):
    truncated = text if len(text) < 30000 else text[:30000]
    prompt = (
        f"You are a helpful translator. Translate the following text to {target_lang_name} (language code: {target_code}). "
        "Do not add explanations or extra commentary. Preserve meaning, punctuation and line breaks when appropriate.\n\n"
        f"Text:\n{truncated}"
    )
    return gemini_generate(prompt, max_output_tokens=1500, temperature=0.1)

def gemini_summarize(text, target_lang_name, target_code):
    truncated = text if len(text) < 30000 else text[:30000]
    prompt = (
        f"You are a concise summarizer. Summarize the following text in {target_lang_name} (language code: {target_code}). "
        "Produce a short, clear summary in up to 6 bullet points. Keep it factual and neutral. Output only the summary.\n\n"
        f"Text:\n{truncated}"
    )
    return gemini_generate(prompt, max_output_tokens=800, temperature=0.2)
    ...
    try:
        ...
        def do_translate(chat_id, orig_message_id, progress_message_id):
            try:
                ...
                translated = gemini_translate(transcription, lang_name, lang_code)
                if not translated:
                    translated = "❌ Translation service returned empty result."
...
def callback_summarize_in(call):
    try:
        ...
        def do_summarize(chat_id, orig_message_id, progress_message_id):
            try:
                transcription = user_transcriptions[uid][orig_message_id]
                summary = gemini_summarize(transcription, lang_name, lang_code)
                if not summary:
                    summary = "❌ Summarization service returned empty result."
                if len(summary) > 4000:
