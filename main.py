...
def admin_confirm_and_send(call):
    ...
    try:
        ...
        user_ids = [int(u["_id"]) for u in user_cursor]
        group_ids = [int(g["_id"]) for g in group_cursor]
        ...
        def send_to_chat(chat_id):
            ...
            try:
                ...
                if len(fail_examples) < 5:
                    fail_examples.append(f"{chat_id}: {str(e)}")
        ...
        try:
            bot.send_message(call.message.chat.id, "❌ An error occurred while broadcasting ads. See logs.")
        except Exception:
            pass
