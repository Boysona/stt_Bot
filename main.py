# main.py  (updated: use slam-1 for English, best for other languages; includes preview deletion + detailed callback logging)
import os
import uuid
import logging
import requests
...
# Configure logging (INFO by default; change to DEBUG for more verbosity)
...
def log_callback(call):
    """Detailed logging for callback queries."""
    try:
...
# Restrict admin menu handler to only the exact menu callback values to avoid intercepting confirm/cancel
...
def admin_menu_callback(call):
    ...
    try:
        ...
        if data == "total_users":
            ...
            # set awaiting state; preview_message_id will be set after admin sends the preview message
...
def admin_confirm_broadcast(call):
    ...
    try:
        ...
        # delete the preview message if exists (cleanup)
...
def prepare_broadcast_message(message):
    try:
        # Save pending broadcast and store preview message id after sending preview
        ...
        # store preview message id for cleanup later
...
def handle_media_common(message, target_bot: telebot.TeleBot):
    ...
    try:
        ...
        # select appropriate AssemblyAI model: slam-1 for English, best for others
...
# HTML_TEMPLATE unchanged (omitted here for brevity in this comment block) 
# But included fully below for completeness in the delivered file.
...
def upload_large_file(token):
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, b, upload_msg_id_inner):
        try:
            ...
            # choose speech_model according to language: slam-1 for English, best for others
